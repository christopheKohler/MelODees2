...
oct_node nearest_color(int *v, node_heap *h)
{
  ...
  for (i = 1; i < h->n; i++) 
  {
    diff = 3 * abs(h->buf[i]->r - v[0]) + 5 * abs(h->buf[i]->g - v[1]) + 2 * abs(h->buf[i]->b - v[2]);
    if (diff < max) 
    {
      max = diff;
      o = h->buf[i];
...
void color_quant(image im, int n_colors, unsigned char* result_colors)
{
  ...
  // Here we got 7 colors.
  // From index 1 to n_colors
  for (i = 1; i < (n_colors+1); i++)
  {
    result_colors[0] = heap.buf[i]->r;
    result_colors[1] = heap.buf[i]->g;
    result_colors[2] = heap.buf[i]->b;
    result_colors += 3;
  }
...
void image_quant(image im, int n_colors, int dither)
{
	int i;
	unsigned char* pix = im->pix;
	node_heap heap = { 0, 0, 0 };
...
int main_quantizeimage()
{
  ...
  int nbcol = 8;
	...
	image im = read_ppm("c:\\Dev\\AmigaDemo2018\\Test.ppm");
	color_quant(im, nbcol, 0);
