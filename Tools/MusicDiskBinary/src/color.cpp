...
#pragma warning( disable : 4244 ) // warning C4244: '=': conversion from 'float' to 'int'
...
void color::writetostreamas16bits(std::ofstream& outputfile)
{
  int value;
  value = ((r >> 4) << 8) | ((g >> 4) << 4) | ((b >> 4));
  ...
  outputfile.write(ptr + 1, 1);
  outputfile.write(ptr + 0, 1);
...
int color::getdiffwitheyeperception(color othercolor)
{
  // Get diff of 2 colors
  int diff;
